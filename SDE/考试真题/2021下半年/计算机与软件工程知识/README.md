...
***
22、下图所示为一个非确定有限自动机（NFA），S0为初态，S3为终态。该NFA识别的字符串（  ）。
...
***
60、n个关键码构成的序列{k1,k2, ...Kn}当且仅当满足下列关系时称其为堆。
...
***
66、用户在电子商务网站上使用网上银行支付时，必须通过(  )在 Internet 与银行专用网之间进行数据交换。
